...
export default function StatusScreen({ config, onReset }: Props) {
  ...
  const [batteryOptDisabled, setBatteryOptDisabled] = useState<boolean | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const startForegroundService = async () => {
      ...
      const foregroundServiceType =
        androidApi >= 34 ? "specialUse" : "dataSync";
      ...
      if (!notificationPermissionGranted) {
        addLog(
          "[warning] Notification permission was not granted. Android may limit background printing.",
        );
      }
      ...
      try {
        ...
        addLog(
          `[warning] Foreground service failed: ${getErrorMessage(error)}`,
        );
      }
    };
    ...
    startSocketService(config, (update) => {
      ...
      addLog(
        `[${update.status}] ${update.message}${update.hint ? " — " + update.hint : ""}`,
      );
    });
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        {hint ? (
          <Text
            style={[styles.hint, status === "error" ? styles.hintError : null]}
          >
            {hint}
          </Text>
        ) : null}
      ...
      {batteryOptDisabled === false && (
        ...
        >
          <Text style={styles.batteryWarningTitle}>
            Battery exemption not confirmed
          </Text>
          <Text style={styles.batteryWarningText}>
            Android has not confirmed this app is exempt from Doze battery
            optimization. Tap here to request the exemption.
          </Text>
        </TouchableOpacity>
      )}
