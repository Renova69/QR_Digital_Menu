...
describe('AuthService', () => {
  ...
  describe('validateUser', () => {
    ...
    it('throws UnauthorizedException when password is wrong', async () => {
      ...
      await expect(service.validateUser('user@example.com', 'wrong')).rejects.toThrow(UnauthorizedException);
    });
  ...
  describe('login', () => {
    it('returns JWT token and user object', async () => {
      ...
      expect(mockJwt.sign).toHaveBeenCalledWith({ email: user.email, sub: user.id });
    });
  });
  ...
  describe('validateGoogleUser', () => {
    ...
    it('creates a new user when not found', async () => {
      ...
      expect(mockUsersService.create).toHaveBeenCalledWith(
        expect.objectContaining({ email: 'new@example.com', name: 'Google User', role: 'OWNER' }),
      );
  ...
  describe('register', () => {
    ...
    it('creates user and returns JWT + user on success', async () => {
      mockUsersService.findByEmail.mockResolvedValue(null);
      mockUsersService.create.mockResolvedValue(makeUser({ password: 'hashed' }));
  ...
  describe('pinLogin', () => {
    ...
    it('throws HttpException(429) when a candidate is locked', async () => {
      ...
      mockPrisma.user.findMany.mockResolvedValue([
        makeUser({ pinHash: 'hash', pinAttempts: 5, pinLockedUntil: futureDate }),
      ]);
      await expect(service.pinLogin('rest1', '1234')).rejects.toThrow(HttpException);
    });
    ...
    it('returns JWT and resets attempts on valid PIN', async () => {
      ...
      expect(mockPrisma.user.updateMany).toHaveBeenCalledWith(
        expect.objectContaining({ data: { pinAttempts: 0, pinLockedUntil: null } }),
      );
    });
    ...
    it('increments attempts and throws UnauthorizedException on wrong PIN', async () => {
      ...
      await expect(service.pinLogin('rest1', 'wrong')).rejects.toThrow(UnauthorizedException);
      expect(mockPrisma.user.updateMany).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ pinAttempts: 1 }) }),
      );
    });
    ...
    it('locks account and throws HttpException(429) after MAX_ATTEMPTS', async () => {
      ...
      await expect(service.pinLogin('rest1', 'wrong')).rejects.toThrow(HttpException);
      expect(mockPrisma.user.updateMany).toHaveBeenCalledWith(
  ...
  describe('sendOtp', () => {
    it('throws HttpException(400) when neither email nor phone provided', async () => {
      await expect(service.sendOtp(undefined, undefined)).rejects.toThrow(HttpException);
    });
    ...
    it('throws HttpException(501) when phone provided but Twilio not configured', async () => {
      ...
      await expect(service.sendOtp(undefined, '+1234567890')).rejects.toThrow(HttpException);
    ...
    it('throws HttpException(429) when token created within last 60 seconds', async () => {
      mockPrisma.verificationToken.findFirst.mockResolvedValue({ id: 'tok1', createdAt: new Date() });
      await expect(service.sendOtp('user@example.com')).rejects.toThrow(HttpException);
    });
  ...
  describe('verifyOtp', () => {
    it('throws HttpException(400) when code is not provided', async () => {
      await expect(service.verifyOtp('user@example.com', undefined)).rejects.toThrow(HttpException);
    });

    it('throws HttpException(400) when email is missing in email flow', async () => {
      await expect(service.verifyOtp(undefined, '123456')).rejects.toThrow(HttpException);
    });

    it('throws UnauthorizedException when no valid token record exists', async () => {
      mockPrisma.verificationToken.findFirst.mockResolvedValue(null);
      await expect(service.verifyOtp('user@example.com', '123456')).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('returns isNew:false for an existing user', async () => {
      ...
      mockUsersService.findByEmail.mockResolvedValue(makeUser({ phone: null, name: 'Existing' }));
