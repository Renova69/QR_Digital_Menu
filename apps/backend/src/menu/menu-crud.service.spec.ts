...
describe('MenuCrudService', () => {
  ...
  describe('getPublicMenu', () => {
    ...
    it('does not call applyLazyTranslations when lang not in targetLanguages', async () => {
      mockPrisma.restaurant.findUnique.mockResolvedValue({ ...BASE_RESTAURANT, targetLanguages: [] });
      mockPrisma.menuCategory.findMany.mockResolvedValue([makeCategory()]);
  ...
  describe('getPublicMenuMeta', () => {
    it('throws NotFoundException when restaurant not found', async () => {
      ...
      await expect(service.getPublicMenuMeta('missing')).rejects.toThrow(NotFoundException);
    });
  ...
  describe('getCategoryItems', () => {
    it('throws NotFoundException when category not found', async () => {
      ...
      await expect(service.getCategoryItems('rest-1', 'missing')).rejects.toThrow(NotFoundException);
    });
  ...
  describe('getTrendingItems', () => {
    ...
    it('returns [] when trendingMode is OFF', async () => {
      mockPrisma.restaurant.findUnique.mockResolvedValue({ trendingMode: 'OFF', id: 'rest-1' });
    ...
    it('returns featured items when trendingMode is MANUAL', async () => {
      ...
      expect(mockPrisma.menuItem.findMany).toHaveBeenCalledWith(
        expect.objectContaining({ where: { category: { restaurantId: 'rest-1' }, isFeatured: true, isOutOfStock: false } }),
      );
      expect(result).toHaveLength(1);
    });
  ...
  describe('createCategory', () => {
    ...
    it('throws ForbiddenException when not restaurant owner', async () => {
      ...
      await expect(
        service.createCategory('rest-1', { name: 'Drinks' } as any, 'other-user'),
    ...
    it('creates and returns category', async () => {
      ...
      mockPrisma.menuCategory.create.mockResolvedValue(makeCategory({ order: 2 }));

      const result = await service.createCategory('rest-1', { name: 'Starters' } as any, 'user-1');

      expect(mockPrisma.menuCategory.create).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ restaurantId: 'rest-1', order: 2 }) }),
      );
  ...
  describe('findAllCategories', () => {
    ...
    it('throws ForbiddenException for non-owner', async () => {
      ...
      await expect(service.findAllCategories('rest-1', 'other')).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('updateCategory', () => {
    ...
    it('updates and returns category', async () => {
      mockPrisma.menuCategory.findUnique.mockResolvedValue(makeCategory());
      mockPrisma.restaurant.findUnique.mockResolvedValue(BASE_RESTAURANT);
      mockPrisma.menuCategory.update.mockResolvedValue({ ...makeCategory(), name: 'Updated' });

      const result = await service.updateCategory('cat-1', { name: 'Updated' } as any, 'user-1');
  ...
  describe('updateCategoryOrder', () => {
    it('reorders via transaction and returns success', async () => {
      ...
      const result = await service.updateCategoryOrder('rest-1', ['cat-1', 'cat-2'], 'user-1');
  ...
  describe('removeCategory', () => {
    it('throws NotFoundException when category not found', async () => {
      ...
      await expect(service.removeCategory('missing', 'user-1')).rejects.toThrow(NotFoundException);
    });

    it('deletes category', async () => {
      mockPrisma.menuCategory.findUnique.mockResolvedValue({ restaurantId: 'rest-1' });
      mockPrisma.restaurant.findUnique.mockResolvedValue(BASE_RESTAURANT);
      mockPrisma.menuCategory.delete.mockResolvedValue(makeCategory());

      await service.removeCategory('cat-1', 'user-1');

      expect(mockPrisma.menuCategory.delete).toHaveBeenCalledWith({ where: { id: 'cat-1' } });
    });
  });
  ...
  describe('updateCategoryImage', () => {
    ...
    it('updates imageUrl and thumbnailUrl', async () => {
      ...
      expect(mockPrisma.menuCategory.update).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ imageUrl: 'url', thumbnailUrl: 'thumb' }) }),
      );
    });
  });
  ...
  describe('createItem', () => {
    it('throws NotFoundException when category not found', async () => {
      ...
      await expect(
        service.createItem('missing', { name: 'Soup', price: 5 } as any, 'user-1'),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('creates item with correct order index', async () => {
      ...
      const result = await service.createItem('cat-1', { name: 'Soup', price: 5 } as any, 'user-1');

      expect(mockPrisma.menuItem.create).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ categoryId: 'cat-1', order: 3 }) }),
      );
  ...
  describe('findAllItemsInCategory', () => {
    it('throws NotFoundException when category not found', async () => {
      ...
      await expect(service.findAllItemsInCategory('missing', 'user-1')).rejects.toThrow(NotFoundException);
    });
  ...
  describe('updateItem', () => {
    ...
    it('updates and returns item', async () => {
      ...
      mockPrisma.restaurant.findUnique.mockResolvedValue(BASE_RESTAURANT);
      mockPrisma.menuItem.update.mockResolvedValue({ ...makeItem(), name: 'Updated' });

      const result = await service.updateItem('item-1', { name: 'Updated' } as any, 'user-1');
  ...
  describe('updateItemImage', () => {
    ...
    it('updates image fields', async () => {
      ...
      expect(mockPrisma.menuItem.update).toHaveBeenCalledWith(
        expect.objectContaining({ data: { imageUrl: 'url', thumbnailUrl: 'thumb' } }),
      );
    });
  });
  ...
  describe('updateItemOrder', () => {
    ...
    it('reorders items via transaction', async () => {
      ...
      const result = await service.updateItemOrder('cat-1', ['item-1'], 'user-1');
  ...
  describe('removeItem', () => {
    it('throws NotFoundException when item not found', async () => {
      ...
      await expect(service.removeItem('missing', 'user-1')).rejects.toThrow(NotFoundException);
    });
    ...
    it('removes itemId from relatedItemIds of other items before deleting', async () => {
      ...
      expect(mockPrisma.menuItem.update).toHaveBeenCalledWith(
        expect.objectContaining({ where: { id: 'item-2' }, data: { relatedItemIds: ['item-3'] } }),
      );
      expect(mockPrisma.menuItem.delete).toHaveBeenCalledWith({ where: { id: 'item-1' } });
    });
  ...
  describe('createMenuOption', () => {
    it('throws NotFoundException when item not found', async () => {
      ...
      await expect(
        service.createMenuOption('missing', { name: 'Size', choices: '[]' } as any, 'user-1'),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('throws BadRequestException when choices is not a JSON array', async () => {
      ...
      await expect(
        service.createMenuOption('item-1', { name: 'Size', choices: '{"key":"val"}' } as any, 'user-1'),
      ).rejects.toThrow(BadRequestException);
    });

    it('creates and returns option', async () => {
      const option = { id: 'opt-1', name: 'Size', choices: [], menuItemId: 'item-1' };
      mockPrisma.menuItem.findUnique.mockResolvedValue(makeItem());
      mockPrisma.restaurant.findUnique.mockResolvedValue(BASE_RESTAURANT);
      ...
      const result = await service.createMenuOption(
        'item-1',
        { name: 'Size', choices: '[{"name":"Small","priceModifier":0}]' } as any,
        'user-1',
      );
  ...
  describe('updateMenuOption', () => {
    ...
    it('throws BadRequestException when choices is invalid JSON array', async () => {
      ...
      await expect(
        service.updateMenuOption('opt-1', { choices: '"not-array"' } as any, 'user-1'),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('updates option', async () => {
      ...
      mockPrisma.restaurant.findUnique.mockResolvedValue(BASE_RESTAURANT);
      mockPrisma.menuOption.update.mockResolvedValue({ id: 'opt-1', name: 'Updated' });

      const result = await service.updateMenuOption('opt-1', { name: 'Updated' } as any, 'user-1');
  ...
  describe('removeMenuOption', () => {
    it('throws NotFoundException when option not found', async () => {
      ...
      await expect(service.removeMenuOption('missing', 'user-1')).rejects.toThrow(NotFoundException);
    });
    ...
    it('deletes option', async () => {
      ...
      expect(mockPrisma.menuOption.delete).toHaveBeenCalledWith({ where: { id: 'opt-1' } });
    });
  });
});
