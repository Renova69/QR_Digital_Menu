...
const makeContext = (opts: { authHeader?: string; restaurantId?: string } = {}): ExecutionContext =>
  ({
    switchToHttp: () => ({
      getRequest: () => ({
...
describe('ApiKeyGuard', () => {
  ...
  it('returns false when authorization header is missing', async () => {
    expect(await guard.canActivate(makeContext({ restaurantId: 'rest-1' }))).toBe(false);
  });

  it('returns false when authorization does not start with Bearer', async () => {
    expect(
      await guard.canActivate(makeContext({ authHeader: 'Basic xyz', restaurantId: 'rest-1' })),
    ).toBe(false);
  });

  it('returns false when token is empty after trimming', async () => {
    expect(
      await guard.canActivate(makeContext({ authHeader: 'Bearer   ', restaurantId: 'rest-1' })),
    ).toBe(false);
  });

  it('returns false when restaurantId param is missing', async () => {
    expect(await guard.canActivate(makeContext({ authHeader: 'Bearer valid-key' }))).toBe(false);
  });

  it('returns false when no restaurant matches the key', async () => {
    mockPrisma.restaurant.findFirst.mockResolvedValue(null);
    expect(
      await guard.canActivate(makeContext({ authHeader: 'Bearer wrong-key', restaurantId: 'rest-1' })),
    ).toBe(false);
  });
