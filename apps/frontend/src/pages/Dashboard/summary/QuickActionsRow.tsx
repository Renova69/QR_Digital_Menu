import { Plus, Monitor, Languages, LucideIcon, ExternalLink } from "lucide-react";
  ...
  return (
    <div className="glass-panel rounded-[1.5rem] p-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {actions.map(({ label, description, Icon, href, external, colorClass, bgClass }) => {
          const content = (
            <>
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center border ${bgClass} group-hover:scale-110 transition-transform`}>
                <Icon className={`w-4 h-4 ${colorClass}`} />
              </div>
              <div className="flex flex-col items-center mt-1.5">
                <span className="text-xs font-semibold text-foreground leading-tight">{label}</span>
                <span className="text-[10px] text-muted-foreground font-medium mt-0.5">{description}</span>
              </div>
            </>
          );

          const className = "flex flex-col items-center gap-1 p-3 rounded-xl bg-secondary/50 hover:bg-secondary transition-colors text-center group cursor-pointer";
