...
const statusConfig: Record<string, { bg: string; text: string; label: string }> = {
  empty: { bg: 'bg-emerald-500/10', text: 'text-emerald-500', label: 'Available' },
  occupied: { bg: 'bg-primary/10', text: 'text-primary', label: 'Occupied' },
  paid: { bg: 'bg-amber-500/10', text: 'text-amber-500', label: 'Paid' },
  waiting: { bg: 'bg-sky-500/10', text: 'text-sky-500', label: 'Waiting' },
};

const LiveTablesGrid = ({ tables }: LiveTablesGridProps) => (
  <div className="glass-panel rounded-[1.5rem] p-5">
    <h3 className="text-sm font-display font-bold text-foreground mb-4">Live Tables</h3>
    {tables.length === 0 ? (
            ...
            >
              ...
              {table.status !== 'empty' && (
                <div className="mt-1.5 flex flex-col gap-0.5">
                  {table.orderCount > 0 && (
                    <span className="text-[10px] text-muted-foreground">{table.orderCount} orders</span>
                  )}
