...
const PaymentSettingsTab: React.FC = () => {
  ...
  const [epayPage, setEpayPage] = useState<"credit_paydirect" | "paylogin">(
    "credit_paydirect",
  );
  ...
  const [myposPrivateKeyConfigured, setMyposPrivateKeyConfigured] =
    useState(false);
  ...
  useEffect(() => {
    if (
      ...
      setMyposPrivateKey("");
      setMyposPrivateKeyConfigured(
        activeRestaurant.myposPrivateKeyConfigured ?? false,
      );
      ...
      setNotifyAllStaffOnPayment(
        activeRestaurant.notifyAllStaffOnPayment ?? true,
      );
  ...
  const handleAddTip = () => {
    ...
    if (!newTipOption || isNaN(v) || v < 1 || v > 100) {
      setTipError(
        t("payment.settings.tipValidationError", {
          defaultValue: "Enter a value between 1 and 100.",
        }),
      );
      return;
    }
    if (tipOptions.includes(v)) {
      setTipError(
        t("payment.settings.tipDuplicateError", {
          defaultValue: "That % is already added.",
        }),
      );
      return;
    }
  ...
  const handleStripeConnect = async () => {
    ...
    try {
      ...
      setStripeError(
        t("payment.settings.connectError", {
          defaultValue: "Failed to connect Stripe. Try again.",
        }),
      );
  ...
  const pills = [
    {
      label: paymentsEnabled
        ? t("payment.settings.statusPaymentsOn", {
            defaultValue: "Payments ON",
          })
        : t("payment.settings.statusPaymentsOff", {
            defaultValue: "Payments OFF",
          }),
      ...
      ? [
          {
            label: stripeOnboarded
              ? t("payment.settings.statusStripeConnected", {
                  defaultValue: "Stripe connected",
                })
              : t("payment.settings.statusStripeNotConnected", {
                  defaultValue: "Stripe not connected",
                }),
            active: stripeOnboarded,
          },
          {
            label: epayConfigured
              ? t("payment.settings.statusEpayConfigured", {
                  defaultValue: "ePay.bg configured",
                })
              : t("payment.settings.statusEpayNotConfigured", {
                  defaultValue: "ePay.bg not configured",
                }),
            active: epayConfigured,
          },
          {
            label: boricaConfigured
              ? t("payment.settings.statusBoricaConfigured", {
                  defaultValue: "BORICA configured",
                })
              : t("payment.settings.statusBoricaNotConfigured", {
                  defaultValue: "BORICA not configured",
                }),
            active: boricaConfigured,
          },
          {
            label: myposConfigured
              ? t("payment.settings.statusMyposConfigured", {
                  defaultValue: "myPOS configured",
                })
              : t("payment.settings.statusMyposNotConfigured", {
                  defaultValue: "myPOS not configured",
                }),
          ...
          },
          {
            label: tipsEnabled
              ...
              : t("payment.settings.statusTipsOff", {
                  defaultValue: "Tips OFF",
                }),
            active: tipsEnabled,
          },
        ]
    ...
    <form onSubmit={handleSave} className="space-y-6">
        ...
        {paymentsEnabled &&
          ...
          !myposConfigured && (
            <p className="mt-3 text-sm text-amber-600 bg-amber-50 dark:bg-amber-950 px-3 py-2 rounded-lg flex items-center gap-1.5">
              ...
              {t("payment.settings.configureProviderWarning", {
                defaultValue:
                  "Connect Stripe or configure ePay.bg/BORICA/myPOS before accepting online payments.",
              })}
            </p>
          )}
      </div>
      ...
      {paymentsEnabled && isEpayFeature && (
        <div className="border-b border-border pb-6">
          <h3 className={`${sectionHeading} mb-4`}>
            {t("payment.settings.stripeConnect")}
          </h3>
          {stripeOnboarded ? (
                ...
                {!disconnectConfirming ? (
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-muted-foreground">
                      {t("payment.settings.disconnectAreYouSure", {
                        defaultValue: "Disconnect Stripe?",
                      })}
                    </span>
                    <button
                      type="button"
                    ...
                    >
                      {disconnecting
                        ? "..."
                        : t("payment.settings.disconnectConfirmBtn", {
        ...
        <div className="border-b border-border pb-6">
          <div className="flex items-center justify-between mb-2">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {t("payment.settings.epayDesc", {
                  defaultValue:
                    "Hosted checkout credentials for Bulgarian card payments.",
                })}
              </p>
            </div>
            <ToggleSwitch
          ...
          {epayEnabled && (
            <span
              className={`inline-flex items-center gap-1.5 text-sm font-medium mb-3 ${epayConfigured ? "text-green-600 dark:text-green-400" : "text-amber-600 dark:text-amber-400"}`}
            >
              <CheckCircle2 className="w-4 h-4" />
              {epayConfigured
                ? t("payment.settings.epayConfigured", {
                    defaultValue: "ePay.bg Configured",
                  })
                : t("payment.settings.epayIncomplete", {
                    defaultValue: "ePay.bg — credentials incomplete",
                  })}
            </span>
          )}
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="space-y-1">
                ...
                <select
                  value={epayMode}
                  onChange={(e) =>
                    setEpayMode(e.target.value as "DEMO" | "LIVE")
                  }
                  className={inputCls}
                >
                  <option value="DEMO">
                    {t("payment.settings.epayDemo", { defaultValue: "Demo" })}
                  </option>
                  <option value="LIVE">
                    {t("payment.settings.epayLive", { defaultValue: "Live" })}
                  </option>
                </select>
              </label>
              ...
              <label className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEpayPage(
                      e.target.value as "credit_paydirect" | "paylogin",
                    )
                  }
                  className={inputCls}
                >
                  <option value="credit_paydirect">
                    {t("payment.settings.epayDirectCard", {
                      defaultValue: "Direct card",
                    })}
                  </option>
                  <option value="paylogin">
                    {t("payment.settings.epayLogin", {
                      defaultValue: "ePay.bg account",
                    })}
                  </option>
                </select>
              </label>
              ...
              <label className="space-y-1">
                <span className="text-xs font-medium text-muted-foreground">
                  {t("payment.settings.epayClientId", {
                    defaultValue: "Merchant CIN/MIN",
                  })}
                </span>
                <input
                  value={epayClientId}
              ...
              <label className="space-y-1 sm:col-span-2">
                <span className="text-xs font-medium text-muted-foreground">
                  {t("payment.settings.epaySecret", {
                    defaultValue: "Secret word",
                  })}
                </span>
                <input
                  type="password"
                ...
                {epaySecretConfigured && (
                  <p className="text-xs text-muted-foreground">
                    {t("payment.settings.epaySecretConfigured", {
                      defaultValue:
                        "Secret saved. Leave blank to keep it unchanged.",
                    })}
                  </p>
                )}
              </label>
        ...
        <div className="border-b border-border pb-6">
          <div className="flex items-center justify-between mb-2">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {t("payment.settings.boricaDesc", {
                  defaultValue:
                    "Direct card payments via BORICA EMV 3DS. Use DEMO mode with the public sandbox for testing — no company account required.",
                })}
              </p>
            </div>
            <ToggleSwitch
          ...
          {boricaEnabled && (
            <span
              className={`inline-flex items-center gap-1.5 text-sm font-medium mb-3 ${boricaConfigured ? "text-green-600 dark:text-green-400" : "text-amber-600 dark:text-amber-400"}`}
            >
              <CheckCircle2 className="w-4 h-4" />
              {boricaConfigured
                ? t("payment.settings.boricaConfigured", {
                    defaultValue: "BORICA Configured",
                  })
                : t("payment.settings.boricaIncomplete", {
                    defaultValue: "BORICA — credentials incomplete",
                  })}
            </span>
          )}
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="space-y-1">
                ...
                <select
                  value={boricaMode}
                  onChange={(e) =>
                    setBoricaMode(e.target.value as "DEMO" | "LIVE")
                  }
                  className={inputCls}
                >
                  <option value="DEMO">
                    {t("payment.settings.boricaDemo", {
                      defaultValue: "Demo (sandbox)",
                    })}
                  </option>
                  <option value="LIVE">
                    {t("payment.settings.boricaLive", { defaultValue: "Live" })}
                  </option>
                </select>
              </label>
              ...
              <div className="space-y-1">
                <span className="text-xs font-medium text-muted-foreground">
                  {t("payment.settings.boricaCurrency", {
                    defaultValue: "Currency",
                  })}
                </span>
                <p className="text-sm text-foreground px-3 py-2 border border-border rounded-lg bg-muted/40">
                  {t("auto.eUR", "EUR")}
                  {t("payment.settings.boricaCurrencyNote", {
                    defaultValue: "(only EUR is supported)",
                  })}
                </p>
              </div>
              ...
              {boricaMode === "LIVE" && (
                <>
                  <label className="space-y-1">
                    <span className="text-xs font-medium text-muted-foreground">
                      {t("payment.settings.boricaTerminalId", {
                        defaultValue: "Terminal ID",
                      })}
                    </span>
                    <input
                      value={boricaTerminalId}
                      onChange={(e) =>
                        setBoricaTerminalId(e.target.value.trim())
                      }
                      className={inputCls}
                  ...
                  <label className="space-y-1">
                    <span className="text-xs font-medium text-muted-foreground">
                      {t("payment.settings.boricaMerchantId", {
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setBoricaMerchantId(e.target.value.trim())
                      }
                      className={inputCls}
                  ...
                  <label className="space-y-1 sm:col-span-2">
                    <span className="text-xs font-medium text-muted-foreground">
                      {t("payment.settings.boricaMerchantName", {
                        defaultValue: "Merchant name (shown on BORICA page)",
                      })}
                    </span>
                    <input
                  ...
                  <label className="space-y-1 sm:col-span-2">
                    <span className="text-xs font-medium text-muted-foreground">
                      {t("payment.settings.boricaPrivateKey", {
                        defaultValue: "Merchant private key (PEM)",
                      })}
                    </span>
                    <textarea
                    ...
                    {boricaPrivateKeyConfigured && (
                      <p className="text-xs text-muted-foreground">
                        {t("payment.settings.boricaKeyConfigured", {
                          defaultValue:
                            "Key saved. Leave blank to keep it unchanged.",
                        })}
                      </p>
                    )}
                  </label>
                  ...
                  <label className="space-y-1 sm:col-span-2">
                    <span className="text-xs font-medium text-muted-foreground">
                      {t("payment.settings.boricaPublicCert", {
                        defaultValue: "BORICA public certificate (PEM)",
                      })}
                    </span>
                    <textarea
              ...
              {boricaMode === "DEMO" && (
                <p className="text-xs text-muted-foreground sm:col-span-2">
                  {t("payment.settings.boricaDemoNote", {
                    defaultValue:
                      "DEMO mode uses the platform's public sandbox terminal. No keys required. Test with cards 5100770000000022 (Mastercard) or 4341792000000044 (Visa).",
                  })}
                </p>
              )}
            </div>
        ...
        <div className="border-b border-border pb-6">
          <div className="flex items-center justify-between mb-2">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {t("payment.settings.myposDesc", {
                  defaultValue:
                    "Hosted card payments via myPOS Checkout. Demo uses sandbox credentials; Live requires a verified myPOS merchant account and Checkout store keys.",
                })}
              </p>
            </div>
            <ToggleSwitch
              checked={myposEnabled}
              onChange={setMyposEnabled}
              aria-label="myPOS"
            />
          </div>
          {myposEnabled && (
            <span
              className={`inline-flex items-center gap-1.5 text-sm font-medium mb-3 ${myposConfigured ? "text-green-600 dark:text-green-400" : "text-amber-600 dark:text-amber-400"}`}
            >
              <CheckCircle2 className="w-4 h-4" />
              {myposConfigured
                ? t("payment.settings.myposConfigured", {
                    defaultValue: "myPOS Configured",
                  })
                : t("payment.settings.myposIncomplete", {
                    defaultValue: "myPOS credentials incomplete",
                  })}
            </span>
          )}
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="space-y-1">
                ...
                <select
                  value={myposMode}
                  onChange={(e) =>
                    setMyposMode(e.target.value as "DEMO" | "LIVE")
                  }
                  className={inputCls}
                >
                  <option value="DEMO">
                    {t("payment.settings.myposDemo", {
                      defaultValue: "Demo (sandbox)",
                    })}
                  </option>
                  <option value="LIVE">
                    {t("payment.settings.myposLive", { defaultValue: "Live" })}
                  </option>
                </select>
              </label>
              ...
              <div className="space-y-1">
                <span className="text-xs font-medium text-muted-foreground">
                  {t("payment.settings.myposCurrency", {
                    defaultValue: "Currency",
                  })}
                </span>
                <p className="text-sm text-foreground px-3 py-2 border border-border rounded-lg bg-muted/40">
                  {t("auto.eUR", "EUR")}
                  {t("payment.settings.myposCurrencyNote", {
                    defaultValue: "(only EUR is supported)",
                  })}
                </p>
              </div>
              ...
              {myposMode === "LIVE" && (
                <>
                  <label className="space-y-1">
                    <span className="text-xs font-medium text-muted-foreground">
                      {t("payment.settings.myposClientNumber", {
                        defaultValue: "Client number / Wallet number",
                      })}
                    </span>
                    <input
                      value={myposClientNumber}
                      onChange={(e) =>
                        setMyposClientNumber(e.target.value.trim())
                      }
                      className={inputCls}
                      placeholder={t("auto.eG61938166610", "e.g. 61938166610")}
                    />
                  </label>

                  <label className="space-y-1">
                    <span className="text-xs font-medium text-muted-foreground">
                      {t("payment.settings.myposStoreId", {
                        defaultValue: "Store ID",
                      })}
                    </span>
                    <input
                      value={myposStoreId}
                      onChange={(e) => setMyposStoreId(e.target.value.trim())}
                      className={inputCls}
                      placeholder={t(
                        "auto.eG000000000000010",
                        "e.g. 000000000000010",
                      )}
                    />
                  </label>
                  ...
                  <label className="space-y-1">
                    <span className="text-xs font-medium text-muted-foreground">
                      {t("payment.settings.myposKeyIndex", {
                        defaultValue: "Key index",
                      })}
                    </span>
                    <input
                      value={myposKeyIndex}
                      onChange={(e) => setMyposKeyIndex(e.target.value.trim())}
                      className={inputCls}
                      placeholder="1"
                    />
                  </label>
                  ...
                  <div className="space-y-1">
                    <span className="text-xs font-medium text-muted-foreground">
                      {t("payment.settings.myposNotifyUrl", {
                        defaultValue: "Notify URL",
                      })}
                    </span>
                    <p className="text-xs text-muted-foreground px-3 py-2 border border-border rounded-lg bg-muted/40">
                      {t("payment.settings.myposNotifyNote", {
                        defaultValue:
                          "Uses BACKEND_URL + /api/v1/payments/mypos/notify. For Live, myPOS requires HTTPS without a custom port.",
                      })}
                    </p>
                  </div>
                  ...
                  <label className="space-y-1 sm:col-span-2">
                    <span className="text-xs font-medium text-muted-foreground">
                      {t("payment.settings.myposPrivateKey", {
                        defaultValue: "Merchant private key (PEM)",
                      })}
                    </span>
                    <textarea
                    ...
                    {myposPrivateKeyConfigured && (
                      <p className="text-xs text-muted-foreground">
                        {t("payment.settings.myposKeyConfigured", {
                          defaultValue:
                            "Key saved. Leave blank to keep it unchanged.",
                        })}
                      </p>
                  ...
                  <label className="space-y-1 sm:col-span-2">
                    <span className="text-xs font-medium text-muted-foreground">
                      {t("payment.settings.myposPublicCert", {
                        defaultValue: "myPOS public certificate (PEM)",
                      })}
                    </span>
                    <textarea
              ...
              {myposMode === "DEMO" && (
                <p className="text-xs text-muted-foreground sm:col-span-2">
                  {t("payment.settings.myposDemoNote", {
                    defaultValue:
                      "DEMO mode uses the official myPOS sandbox data. No myPOS merchant account or registered company is required for sandbox testing, but the notify URL still needs to be reachable by myPOS.",
                  })}
                </p>
              )}
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2 items-center">
                <input
                  ...
                  value={newTipOption}
                  onChange={(e) => {
                    setNewTipOption(e.target.value);
                    setTipError("");
                  }}
                  onKeyDown={(e) =>
                    e.key === "Enter" && (e.preventDefault(), handleAddTip())
                  }
      ...
      <div className="border-b border-border pb-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className={sectionHeading}>
              {t("payment.settings.notifyStaff", {
                defaultValue: "Payment Notifications",
              })}
            </h3>
            <p className="text-sm text-muted-foreground mt-1">
              {t("payment.settings.notifyStaffDesc", {
                defaultValue:
          ...
          <ToggleSwitch
            ...
            aria-label={t("payment.settings.notifyStaff", {
              defaultValue: "Payment Notifications",
            })}
          />
        </div>
