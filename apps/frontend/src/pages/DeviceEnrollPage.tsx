...
function isAlreadyUsedEnrollmentError(error: unknown) {
  return getApiErrorCode(error) === "ENROLLMENT_LINK_USED";
}

function isSharedDeviceDisabledError(error: unknown) {
...
export default function DeviceEnrollPage() {
  ...
  const [message, setMessage] = useState<EnrollmentMessage>({
    key: "deviceEnrollment.verifying",
  });
  ...
  useEffect(() => {
    if (!token) {
      setState("error");
      setMessage({ key: "deviceEnrollment.missingToken" });
      return;
    }
    ...
    const verify = async () => {
      try {
        ...
        setState("ready");
        setMessage({ key: "deviceEnrollment.linkedOpening" });
        ...
        if (isSharedDeviceDisabledError(err)) {
          ...
          setMessage({ key: details.key, params: details.params });
          return;
        }
        ...
        if (alreadyUsed && existingDevice?.deviceToken === token) {
          ...
          setState("ready");
          setMessage({ key: "deviceEnrollment.alreadyLinkedOpening" });
          redirectTimer = setTimeout(
            () => navigate("/device-login", { replace: true }),
            1000,
          );
        ...
        if (alreadyUsed && !hasUsableDeviceBond) {
          setState("error");
          setMessage({ key: "deviceEnrollment.usedOnAnotherDevice" });
          return;
        }
      ...
      <div className="w-full max-w-sm text-center">
        ...
        <h1 className="text-xl font-semibold mb-2">
          {state === "shared-device-off"
            ? t("deviceEnrollment.sharedDeviceModeOffTitle")
            : state === "error"
              ? t("deviceEnrollment.linkFailedTitle")
              : state === "ready"
                ? t("deviceEnrollment.linkedTitle")
                : t("deviceEnrollment.linkingTitle")}
        </h1>
        ...
        {(state === "error" || state === "shared-device-off") && (
          <div className="mt-6 space-y-3">
            <p className="text-xs text-slate-500">
              {state === "shared-device-off"
                ? t("deviceEnrollment.enableSharedDeviceMode")
                : t("deviceEnrollment.requestFreshQr")}
            </p>
            <Link
              to="/login"
            ...
            >
              {t("deviceEnrollment.managerLogin")}
            </Link>
          </div>
        )}
      </div>
