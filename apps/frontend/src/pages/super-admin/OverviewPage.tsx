...
function maxTierCount(...groups: Array<TierCounts | undefined>) {
  return Math.max(
    ...
    ...groups.flatMap((group) =>
      TIER_ORDER.map((tier) => countFor(group, tier)),
    ),
  );
}
    ...
    <div className="rounded-lg border border-slate-800 bg-slate-900 p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 text-2xl font-bold leading-none text-white tabular-nums">
            {value}
          </p>
...
function TierBadge({ tier }: { tier: string }) {
  ...
  return (
    <span
      className={`inline-flex rounded-md border px-2 py-0.5 text-[11px] font-bold ${TIER_STYLES[tier] ?? TIER_STYLES.FREE}`}
    >
      {tier}
    </span>
  );
...
function TierComparison({
  ...
  billing: TierCounts;
  effective: TierCounts;
}) {
  const max = maxTierCount(billing, effective);
  const overrideCount = TIER_ORDER.reduce(
    (acc, tier) =>
      acc + Math.max(0, countFor(effective, tier) - countFor(billing, tier)),
    0,
  );
  ...
  return (
    <div className="rounded-lg border border-slate-800 bg-slate-900 p-5">
      <div className="mb-5 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            <span className="text-slate-400">Subscription</span> = what Stripe
            charges. <span className="text-slate-400">Active</span> = what they
            actually get (may differ if manually overridden).
          </p>
        </div>
      ...
      {/* Column headers */}
      <div className="mb-3 grid grid-cols-[88px_1fr_36px_1fr_36px] gap-x-2 text-[11px] font-medium uppercase tracking-wide text-slate-600">
        <span />
      ...
      <div className="space-y-3">
        {TIER_ORDER.map((tier) => {
          ...
          const delta = effectiveCount - billingCount;
          const hasOverride = delta !== 0;

          return (
            <div
              key={tier}
              className="grid grid-cols-[88px_1fr_36px_1fr_36px] items-center gap-x-2"
            >
              <TierBadge tier={tier} />
              ...
              <div className="h-2 rounded-full bg-slate-800">
                <div
                  ...
                  style={{
                    width: `${Math.max(2, (billingCount / max) * 100)}%`,
                  }}
                />
              </div>
              ...
              <div className="h-2 rounded-full bg-slate-800">
                <div
                  className={`h-2 rounded-full ${hasOverride ? "bg-amber-400" : "bg-emerald-400"}`}
                  style={{
                    width: `${Math.max(2, (effectiveCount / max) * 100)}%`,
                  }}
                />
              </div>
              <span
                className={`text-right text-xs tabular-nums font-semibold ${hasOverride ? "text-amber-400" : "text-slate-400"}`}
              >
                {effectiveCount}
              </span>
            </div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-slate-800 pt-3">
        ...
        {overrideCount > 0 && (
          <span className="ml-auto text-[11px] font-semibold text-amber-400">
            {overrideCount} on manual override
          </span>
        )}
      </div>
    </div>
  ...
  return (
    <div className="rounded-lg border border-slate-800 bg-slate-900 p-5">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          <h3 className="text-sm font-semibold text-white">
            {t("auto.forceTierOverrides", "Force Tier Overrides")}
          </h3>
          <p className="mt-1 text-xs text-slate-500">
            {t(
              "auto.manualAccessChangesThatBypassBillin",
              "Manual access changes that bypass billing tier.",
            )}
          </p>
        </div>
      ...
      {neutral > 0 && (
        <p className="mt-3 text-xs text-slate-500">
          {neutral}{" "}
          {t(
            "auto.overrideKeepsTheSameEffectiveTier",
            "override keeps the same effective tier.",
          )}
        </p>
      )}
    </div>
...
function AttentionPanel({
  ...
  stats: NonNullable<
    Awaited<ReturnType<typeof getSuperAdminStats>>["attentionNeeded"]
  >;
}) {
  const { t } = useTranslation();
  const rows = [
    {
      key: "forcedOverrides",
      label: "Forced tier overrides",
      tone: "text-amber-300",
    },
    {
      key: "paymentsNotOnboarded",
      label: "Payments enabled, Stripe missing",
      tone: "text-red-300",
    },
  ...
  return (
    <div className="rounded-lg border border-slate-800 bg-slate-900 p-5">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          <h3 className="text-sm font-semibold text-white">
            {t("auto.attentionNeeded", "Attention Needed")}
          </h3>
          <p className="mt-1 text-xs text-slate-500">
            {t(
              "auto.tenantsWorthCheckingBeforeTheyBecom",
              "Tenants worth checking before they become support work.",
            )}
          </p>
        </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24">
        ...
        <p className="font-medium text-slate-400">
          {t("auto.failedToLoadPlatformStats", "Failed to load platform stats")}
        </p>
        <p className="mt-1 text-sm text-slate-600">
          {t(
            "auto.checkYourConnectionAndTryAgain",
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white">
            {t("auto.overview", "Overview")}
          </h2>
          <p className="mt-1 text-sm text-slate-500">
            {t(
              "auto.platformHealthTierAccessAndTenant",
              "Platform health, tier access, and tenant risks",
            )}
          </p>
        </div>
        <button
