...
const analyticsData: AnalyticsData = {
  ...
  avgOrderValue: 14,
  servedRate: 100,
  ordersByStatus: [{ status: 'SERVED', count: 3 }],
...
describe('XLSX export smoke coverage', () => {
  ...
  it('downloads analytics exports through the browser toFile API', async () => {
    await downloadAnalyticsExport(
      analyticsData,
      { restaurantName: 'Demo Restaurant', startDate: '2026-06-01', endDate: '2026-06-12', period: 7 },
      t,
    );
  ...
  it('downloads payments exports through the browser toFile API', async () => {
    await downloadPaymentsExport([paymentRecord], { restaurantName: 'Demo Restaurant' }, t);

    expect(xlsxMocks.writeXlsxFile).toHaveBeenCalledWith(
      expect.arrayContaining([expect.objectContaining({ sheet: 'Summary' })]),
    );
    expect(xlsxMocks.toFile).toHaveBeenCalledWith('payments-demo-restaurant-2026-06-12.xlsx');
  });
  ...
  it('downloads menu exports through the browser toFile API', async () => {
            ...
            items: [
              {
                name: 'Soup',
                price: 5,
                options: [{ name: 'Size', choices: [{ name: 'Large', priceModifier: 1 }] }],
              },
            ],
          },
        ],
    ...
    expect(xlsxMocks.writeXlsxFile).toHaveBeenCalledWith(
      expect.arrayContaining([expect.objectContaining({ sheet: 'Categories' })]),
    );
    expect(xlsxMocks.toFile).toHaveBeenCalledWith('menu-export-demo-restaurant-2026-06-12.xlsx');
  });
