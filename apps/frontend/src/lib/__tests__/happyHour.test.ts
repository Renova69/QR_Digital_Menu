...
describe("isHappyHourActive", () => {
  ...
  it("returns false when disabled or misconfigured", () => {
    ...
    expect(
      isHappyHourActive({
        ...
        happyHourStartTime: "10:00",
        happyHourEndTime: "12:00",
      }),
    ).toBe(false);
    expect(
      isHappyHourActive({
        ...base,
        happyHourStartTime: "bad",
        happyHourEndTime: "12:00",
      }),
    ).toBe(false);
    expect(
  ...
  it("matches a normal same-day window", () => {
    ...
    expect(
      isHappyHourActive(
        { ...base, happyHourStartTime: "10:00", happyHourEndTime: "12:00" },
        now,
      ),
    ).toBe(true);
    expect(
      isHappyHourActive(
        { ...base, happyHourStartTime: "12:30", happyHourEndTime: "14:00" },
        now,
      ),
    ).toBe(false);
  });
  ...
  it("attributes an overnight window to the START day (mirrors backend L4)", () => {
    ...
    expect(isHappyHourActive({ ...cfg, happyHourDays: [5] }, sat0100)).toBe(
      true,
    );
    // Only Saturday (6) active -> must NOT match (it's the previous day's window).
    expect(isHappyHourActive({ ...cfg, happyHourDays: [6] }, sat0100)).toBe(
      false,
    );
  });
  ...
  it("matches the evening side of an overnight window on the start day", () => {
    ...
    expect(isHappyHourActive({ ...cfg, happyHourDays: [5] }, fri2300)).toBe(
      true,
    );
    expect(isHappyHourActive({ ...cfg, happyHourDays: [6] }, fri2300)).toBe(
      false,
    );
  });
});
