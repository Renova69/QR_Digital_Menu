...
describe("PosSplitDrawer", () => {
  ...
  it("settles selected items via Cash and refreshes the running balance", async () => {
    ...
    // Select 1 × Beer via its labelled add button.
    fireEvent.click(screen.getByRole("button", { name: "Add one Beer" }));
    ...
    await waitFor(() =>
      expect(apiMock.getSessionBill).toHaveBeenCalledTimes(2),
    );
  });
