      ...
      <header className="max-w-7xl mx-auto glass-panel rounded-2xl pointer-events-auto shadow-xl">
          ...
          <div className="flex items-center gap-2">
            ...
            {user ? (
              <>
                {isProfile ? (
                  returnTo && (
                    <Link to={returnTo} className="hidden sm:flex h-9 items-center px-4 rounded-xl border border-border/50 bg-transparent hover:bg-muted text-xs font-bold uppercase tracking-widest text-muted-foreground transition-all">
                      ← {t('nav.menu', 'Menu')}
                    </Link>
                  )
                ) : (
                  <Link to="/dashboard" className="hidden sm:flex h-9 items-center px-4 rounded-xl bg-secondary text-secondary-foreground text-xs font-bold uppercase tracking-widest hover:bg-secondary/80 transition-all">
                    {t('nav.dashboard')}
                  </Link>
                )}
                <button
              ...
              <>
                <Link to="/login" className="hidden sm:flex h-9 items-center px-4 rounded-xl border border-border/50 bg-transparent hover:bg-muted text-xs font-bold uppercase tracking-widest text-muted-foreground transition-all">
                  {t('nav.login')}
                </Link>
                <Link
                  to="/register"
        ...
        {mobileOpen && (
          <div className="md:hidden border-t border-border/30 px-4 py-4 flex flex-col gap-2">
            <Link to="/#features" onClick={() => setMobileOpen(false)} className="px-4 py-2.5 rounded-xl text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-muted transition-all">
              {t('nav.features', 'Features')}
            </Link>
            <Link to="/pricing" onClick={() => setMobileOpen(false)} className="px-4 py-2.5 rounded-xl text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-muted transition-all">
              {t('nav.pricing', 'Pricing')}
            </Link>
            {user ? (
              <>
                <Link to="/dashboard" onClick={() => setMobileOpen(false)} className="px-4 py-2.5 rounded-xl text-sm font-medium text-muted-foreground hover:bg-muted transition-all">
                  {t('nav.dashboard')}
                </Link>
                <button onClick={handleLogout} className="px-4 py-2.5 rounded-xl text-sm font-medium text-left text-destructive hover:bg-destructive/5 transition-all">
                  {t('nav.logout')}
                </button>
              </>
            ) : (
              <>
                <Link to="/login" onClick={() => setMobileOpen(false)} className="px-4 py-2.5 rounded-xl text-sm font-medium text-muted-foreground hover:bg-muted transition-all">
                  {t('nav.login')}
                </Link>
                <Link to="/register" onClick={() => setMobileOpen(false)} className="brand-cta text-white text-sm font-bold px-4 py-2.5 rounded-xl text-center">
