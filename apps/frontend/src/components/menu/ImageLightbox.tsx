...
export const ImageLightbox: React.FC<ImageLightboxProps> = ({ src, alt, onClose }) => {
    ...
    const handleTouchStart = (e: React.TouchEvent) => {
        if (e.touches.length === 2) {
            pinchRef.current = { initialDist: dist(e.touches[0], e.touches[1]), initialScale: scale };
    ...
    const handleTouchMove = (e: React.TouchEvent) => {
        if (e.touches.length === 2 && pinchRef.current) {
            ...
            const newScale = Math.max(1, Math.min(4,
                pinchRef.current.initialScale * (dist(e.touches[0], e.touches[1]) / pinchRef.current.initialDist)
            ));
            setScale(newScale);
    ...
    return createPortal(
        <div
            ...
            style={{
                ...
                transition: isGesturing ? 'none' : 'background-color 0.2s, backdrop-filter 0.2s',
            }}
        >
            {/* Close button */}
            <button
            ...
            <div
                ...
                style={{
                    ...
                    animation: !isGesturing && dragY === 0 && scale === 1
                        ? 'lightboxZoomIn 0.3s cubic-bezier(0.16,1,0.3,1)' : undefined,
                    touchAction: 'none',
