...
export const Modal: React.FC<ModalProps> = ({ open, onOpenChange, children, title, description, trigger }) => {
  return (
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[95vw] max-w-lg glass-panel bg-background p-8 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-2xl z-[1001] border-white/5 animate-in zoom-in-95 fade-in duration-300">
          ...
          <Dialog.Close asChild>
            <button className="absolute top-6 right-6 p-2 rounded-full bg-secondary/50 text-muted-foreground hover:text-foreground transition-all active:scale-90" aria-label="Close">
