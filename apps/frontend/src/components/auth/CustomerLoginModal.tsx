...
export const CustomerLoginModal: React.FC<CustomerLoginModalProps> = ({
  ...
  const fullPhone =
    countryCode + localPhone.replace(/^0+/, "").replace(/\D/g, "");
  ...
  const handleSendCode = async (e: React.FormEvent) => {
    ...
    if (channel === "email") {
      if (!validateEmail(email)) {
        setError(t("auth.otp.invalidEmail", "Enter a valid email address."));
        return;
      }
    } else {
      if (!validatePhone(fullPhone)) {
        setError(t("auth.otp.invalidPhone", "Enter your phone number."));
        return;
      }
    }
    setIsLoading(true);
              ...
              <div className="flex justify-between items-center text-sm">
                ...
                >
                  {channel === "email"
                    ...
                    : t("auth.otp.changePhone", "Change number")}
                </button>
                <button
                  type="button"
