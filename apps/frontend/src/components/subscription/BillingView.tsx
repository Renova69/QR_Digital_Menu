...
const PLAN_CONFIG: Record<
  SubscriptionTier,
  { monthly: number; highlight: FeatureFlag[] }
> = {
  FREE: {
    monthly: 0,
...
const FEATURE_LABELS: Record<FeatureFlag, { key: string; fallback: string }> = {
  ...
  "menu:edit": {
    key: "subscription.features.menuEdit",
    fallback: "Menu management",
  },
  "menu:import": {
    key: "subscription.features.menuImport",
  ...
  "orders:call-waiter": {
    key: "subscription.features.callWaiter",
    fallback: "Call waiter",
  },
  "analytics:basic": {
    key: "subscription.features.analyticsBasic",
    fallback: "Basic analytics",
  },
  "analytics:full": {
    key: "subscription.features.analyticsFull",
    fallback: "Full analytics",
  },
  "payments:stripe": {
    key: "subscription.features.stripePayments",
    fallback: "Stripe payments",
  },
  "payments:epay": {
    key: "subscription.features.epayPayments",
    fallback: "ePay.bg payments",
  },
  "payments:borica": {
    key: "subscription.features.boricaPayments",
    fallback: "BORICA payments",
  },
  "payments:mypos": {
    key: "subscription.features.myposPayments",
    fallback: "myPOS payments",
  },
  "languages:multi": {
    key: "subscription.features.multiLanguage",
    fallback: "Multi-language menu",
  },
  "branding:custom": {
    key: "subscription.features.customBranding",
    fallback: "Custom branding",
  },
  loyalty: {
    key: "subscription.features.loyalty",
    fallback: "Loyalty program",
  },
  "customers:auth": {
    key: "subscription.features.customerAccounts",
    fallback: "Customer accounts",
  },
  upselling: { key: "subscription.features.upselling", fallback: "Upselling" },
  dayparting: {
    key: "subscription.features.dayparting",
    fallback: "Dayparting and happy hour",
  },
  ...
  multilocation: {
    key: "subscription.features.multilocation",
    fallback: "Multi-location",
  },
  "printers:thermal": {
    key: "subscription.features.thermalPrinters",
    fallback: "Thermal printers",
  },
  "templates:menu": {
    key: "subscription.features.menuTemplates",
    fallback: "Menu templates",
  },
  "staff:unlimited": {
    key: "subscription.features.unlimitedStaff",
    fallback: "Unlimited staff",
  },
  "reservations:enabled": {
    key: "subscription.features.reservations",
...
export default function BillingView() {
  ...
  const activeStaffCount = useMemo(
    () =>
      staffMembers.filter(
        (member) => member.role !== "OWNER" && member.isActive !== false,
      ).length,
    [staffMembers],
  );
  ...
  const checkoutMutation = useMutation({
    mutationFn: (targetTier: SubscriptionTier) =>
      createCheckoutSession(targetTier, "monthly", false, activeRestaurant?.id),
    onSuccess: ({ url }) => {
  ...
  const statusLabel = subscription?.cancelAtPeriodEnd
    ...
    : hasSubscription
      ? t(`subscription.status.${subscription?.status ?? "active"}`, {
          defaultValue: t("subscription.status.active", "Active"),
        })
      : tier === "FREE"
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <SummaryCard
          ...
          meta={
            subscription?.cancelAtPeriodEnd
              ? t("subscription.cancelsOn", "Cancels {{date}}", {
                  date: periodDate ?? "-",
                })
              : periodDate
                ? t("subscription.renewsOn", "Renews {{date}}", {
                    date: periodDate,
                  })
        ...
        <SummaryCard
          ...
          meta={
            staffError
              ? t(
                  "subscription.staffUsageUnavailable",
                  "Could not load current usage",
                )
              : t("subscription.activeStaff", "{{count}} active staff", {
        ...
        <SummaryCard
          ...
          value={t("subscription.featuresCount", "{{count}} features", {
            count: features.length,
          })}
          meta={
            nextTier
              ? t("subscription.nextPlan", "Next: {{tier}}", {
                  tier: formatTier(nextTier),
                })
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="space-y-6">
          <section className="rounded-xl border border-border bg-card p-5">
            <div className="mb-4 flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  {t(
                    "subscription.usageDesc",
                    "Track limits that affect access inside this restaurant.",
                  )}
                </p>
              </div>
              {tier !== "FREE" && (
                <button
                  type="button"
                  onClick={() =>
                    navigate("/dashboard?tab=settings&settingsTab=staff")
                  }
            ...
            <div className="rounded-lg border border-border bg-background p-4">
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                {staffLimit === Infinity
                  ? t(
                      "subscription.unlimitedStaffDesc",
                      "This plan does not limit staff seats.",
                    )
                  : t(
                      "subscription.staffUsageDesc",
          ...
          <section className="rounded-xl border border-border bg-card p-5">
            <div className="mb-4">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {t(
                  "subscription.includedFeaturesDesc",
                  "Grouped by the parts of the product they unlock.",
                )}
              </p>
            </div>
                  ...
                  >
                    <div className="mb-3 flex items-center justify-between gap-3">
                      <h4 className="text-sm font-semibold text-foreground">
                        {t(
                          `subscription.featureGroups.${group.key}`,
                          group.key,
                        )}
                      </h4>
        ...
        <aside className="space-y-4">
          <section className="rounded-xl border border-border bg-card p-5">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {t(
                "subscription.billingActionsDesc",
                "Manage your SaaS plan and invoices separately from restaurant payments.",
              )}
            </p>
                ...
                <div className="rounded-lg border border-amber-500/20 bg-amber-500/10 p-3 text-sm text-amber-700 dark:text-amber-300">
                  <div className="flex gap-2">
                    ...
                    <span>
                      {tier === "FREE"
                        ? t(
                            "subscription.freePlanHelp",
                            "Upgrade when you are ready to unlock paid features.",
                          )
                        : t(
                            "subscription.manualPlanHelp",
                            "This restaurant has plan access without an active Stripe subscription.",
                          )}
                    </span>
                  </div>
                </div>
          ...
          {nextTier && (
            <section className="rounded-xl border border-primary/20 bg-primary/5 p-5">
              ...
              <h3 className="mt-2 text-xl font-bold text-foreground">
                {t("subscription.upgradeToTier", "Upgrade to {{tier}}", {
                  tier: formatTier(nextTier),
                })}
              </h3>
              <p className="mt-1 text-sm text-muted-foreground">
                {t(
                  ...
                  "Unlocks {{count}} more capabilities for this restaurant.",
                  {
