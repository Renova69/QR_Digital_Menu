...
export default function SubscriptionBanner() {
  ...
  return (
    <div
      className={`flex items-center justify-between gap-4 px-5 py-3 rounded-2xl mb-6 ${styles.bg} border border-border/40`}
    >
      <div className="flex items-center gap-3 min-w-0">
        <span
          className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shrink-0 ${styles.badge}`}
        >
          {tier}
        </span>
        <span className={`text-xs font-semibold truncate ${styles.text}`}>
          {hasSubscription
            ...
            : tier === "FREE"
              ...
              : t(
                  "subscription.banner.noSubscription",
