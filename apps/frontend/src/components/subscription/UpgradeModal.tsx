...
export default function UpgradeModal({ feature, onClose }: Props) {
  ...
  const minTier: SubscriptionTier = feature
  ...
  const shownTiers = TIERS.filter(
    ({ key }) => TIER_ORDER.indexOf(key) >= minTierIndex,
  );
  const colsClass =
    shownTiers.length >= 3
      ? "sm:grid-cols-3"
  ...
  const { mutate: checkout, isPending } = useMutation({
    mutationFn: (tier: string) =>
      createCheckoutSession(tier, "monthly", false, activeRestaurantId),
    onSuccess: ({ url }) => {
      ...
      <div className="relative z-10 w-full max-w-2xl max-h-[90dvh] overflow-y-auto bg-card border border-border rounded-3xl shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="relative px-5 sm:px-8 pt-8 pb-6 border-b border-border">
          ...
          <h2 className="text-2xl md:text-3xl font-display font-black text-foreground tracking-tight mb-1">
            {t("upgrade.unlockTitle", "Unlock {{feature}}", {
          ...
          <p className="text-sm text-muted-foreground">
            {t(
              "upgrade.subtitle",
              "Available from the {{tier}} plan. Upgrade to unlock this and more.",
              { tier: minTier },
            )}
          </p>
        </div>
        ...
        <div
          className={`px-5 sm:px-8 py-6 grid grid-cols-1 gap-3 ${colsClass}`}
        >
              ...
              >
                {isRecommended && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-10">
                    <span
                      className="text-white px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest shadow whitespace-nowrap"
                ...
                <ul className="flex-1 space-y-1.5 mb-4">
                  {bullets.map((b) => (
                    ...
                    >
                      <Check
                        className={`w-3 h-3 mt-0.5 shrink-0 ${isRecommended ? "text-primary" : "text-muted-foreground/60"}`}
                      />
                      {b}
                    </li>
                  ))}
        ...
        {error && (
          <p className="px-5 sm:px-8 pb-2 text-sm text-destructive text-center">
            {error}
          </p>
        )}
