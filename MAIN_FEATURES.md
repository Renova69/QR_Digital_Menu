...
> **Codebase:** 180+ frontend source files, 32 backend domain modules, 30+ database models, ~400+ i18n keys across 12 languages
...
### 5.17 Platform Settings — `/api/platform-settings/*`

| Method  | Path                       | Auth            | Purpose                                                                                      |
| ------- | -------------------------- | --------------- | -------------------------------------------------------------------------------------------- |
| `GET`   | `/platform-settings`       | **None**        | Public platform config (GDPR toggles, legal content). 30s in-memory cache.                   |
| `GET`   | `/platform-settings/admin` | SUPER_ADMIN JWT | Full settings including all fields.                                                          |
| `PATCH` | `/platform-settings`       | SUPER_ADMIN JWT | Update settings (toggles, per-locale content, retention tunables, data-controller metadata). |

### 5.18 User Data (GDPR) — `/api/users-data/*`

| Method | Path                  | Auth | Purpose                                                                                                                                             |
| ------ | --------------------- | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET`  | `/users-data/export`  | JWT  | Export all personal data as JSON (GDPR Art. 20). Gated by `dataExportEnabled` toggle.                                                               |
| `POST` | `/users-data/erasure` | JWT  | Erase account: anonymise Order PII (tax retention), cascade-delete LoyaltyAccount / tokens (GDPR Art. 17). Gated by `rightToErasureEnabled` toggle. |
...
| BORICA EMV-3DS Payment                  | Complete (June 2026)       | RSA-SHA256 MAC signing, TRTYPE=90 protocol, EUR enforcement, ADDENDUM support. Behind `IPaymentProvider` interface.                                                                                                                                                                                                                                                       |
| ePay.bg Hosted Checkout                 | Complete (June 2026)       | HMAC-signed checkout, callback verification, merchant config, expired-invoice recovery. Independent feature flag.                                                                                                                                                                                                                                                         |
| MyPOS Card Terminal                     | Complete (June 2026)       | Demo/live mode, settlement workflow, card-payment close via POS.                                                                                                                                                                                                                                                                                                          |
| Print Station System                    | Complete (June 2026)       | `PrintStation`, `PrintAgentToken`, `PrintJob` models. ESC/POS ticket builder (Cyrillic). Expo Android agent. Customizable receipt templates. Category-to-station assignment. EventsGateway job routing. `PrintStationsView` dashboard.                                                                                                                                    |
| Analytics Deep-Dive v2/v3               | Complete (June 2026)       | Refund rate, payment split breakdown, repeat customer rate, revenue/hour, revenue reconciliation strip, completion rate KPI, loading skeleton, at-order price snapshots, DB-side aggregations.                                                                                                                                                                            |
| i18n Expansion (12 Locales)             | Complete (June 2026)       | DE, ES, FR, IT, ZH, EL, JA, RU, AR added to EN/BG/RO. DeepL-generated with manual QA. Lazy-loaded bundles (183→54 KB gz). RTL support for AR.                                                                                                                                                                                                                             |
| Split Bill                              | Complete (June 2026)       | PER_ITEM, EVEN, CUSTOM_PARTIAL modes. `SplitProvider` with allocation calculation. Multiple settlements per session. Cash request settlement flow.                                                                                                                                                                                                                        |
| Service Points                          | Complete (June 2026)       | `ServicePoint` model with unique QR codes. Public ordering flow. Dashboard management. Session isolation hardening.                                                                                                                                                                                                                                                       |
| Web Push Notifications                  | Complete (June 2026)       | VAPID-based push. `PushSubscription` model. Service worker via `vite-plugin-pwa`. Notification click routing. Key rotation with auto-resubscribe.                                                                                                                                                                                                                         |
| Context-Aware Upselling                 | Complete (June 2026)       | WeatherAPI integration. AUTO mode trending with bounded queries. Batched public-menu endpoint. Scan retention tracking.                                                                                                                                                                                                                                                   |
| Staff Device Management                 | Complete (June 2026)       | Device enrollment hardening. Shared device mode enforcement. Device count limits. CSPRNG PIN generation. Staff usage tracking.                                                                                                                                                                                                                                            |
| Reservations System                     | Complete (July 2026)       | Public `BookingPage` with calendar/picker. `BookingConfirmationPage` + `BookingManagePage` self-service. Dashboard `ReservationsView` with CRUD + analytics. Seating zones, blackout dates, SMS notifications (Twilio + SIM gateway). 12-lang i18n. Tier-gated (PROFESSIONAL+).                                                                                           |
| Branding/Theme Consolidation            | Complete (July 2026)       | `ThemeContext` + `PosThemeContext`. FOUC fix with per-restaurant inline script. Light default enforcement. Theme persistence per storage key.                                                                                                                                                                                                                             |
| Loyalty Checkout & Redemption           | Complete (July 2026)       | Tier progress bar in CheckoutPage. Reward redemption with deterministic `redeemCartIds`. Automated menu reward pricing. FOR UPDATE locking on ledger writes.                                                                                                                                                                                                              |
| Translation Pipeline Rework             | Complete (July 2026)       | `TranslationJob` sidecar table. Job queue with retry/backoff. Native DeepL glossary. Per-restaurant quota. Progress bar UI. 6 race-condition fixes.                                                                                                                                                                                                                       |
| Allergen/Dietary Tag System             | Complete (July 2026)       | 7 allergen SVG icons + dietary tags. `TagPicker` with search. `MenuItem.tags String[]`. 12-lang i18n. Tap-to-open tooltips. Description expand/collapse.                                                                                                                                                                                                                  |
| Dashboard Polish & Hardening            | Complete (July 2026)       | URL tab persistence. `react-datepicker` localization. Analytics math fixes. POS offline-queue hardening. Payment settlement lock ordering. SWC NestJS compiler (15s → 0.3s). `lazyWithReload` for stale chunks. 21-agent audit (259 findings → 56 fixes).                                                                                                                 |
...
| **POS Integration**     | V4 planned — Square/Toast/Lightspeed. MyPOS already integrated.                                                                                     | Provider abstraction pattern supports additional providers. |
| **KDS Polish**          | Kitchen Display System live at `/staff/kitchen` with real-time kanban board. Could benefit from performance optimizations for high-volume kitchens. | Web Worker filtering, virtualized lists.                    |
| **Service-Level Tests** | Test suite growing — BE ~1200+ tests, FE ~200+ tests. Coverage improving with each audit cycle.                                                     | Reach 80% coverage target, add CI enforcement.              |
...
| 4   | ~~**No service-level unit tests**~~ — **IN PROGRESS.** 22 `.spec.ts` files, 454 tests passing (May 17, 2026). New suites added this sprint. Coverage still below 80% threshold.                                                                                                                                                                                                                                                                                                                                   | Regression risk.                                                         | Reach 80% coverage.                             | Medium     | ~~Must-have~~ **In Progress** |
...
| 8   | ~~**No CI gate on the main branch**~~ — **RESOLVED May 18, 2026.** `.github/workflows/ci.yml` blocks merge on `main`/`master` if any check fails. Checks: backend unit tests (`jest --reporters=default --ci`, bypasses hardcoded Windows path in tdd-guard-jest reporter), frontend typecheck (`tsc --noEmit`, because `vite build` does not type-check), frontend tests (`vitest run`), full monorepo build (`turbo run build`). `DATABASE_URL` dummy value satisfies Prisma format validation without real DB. | Tier/auth regressions reached production undetected (May 16-17 pattern). | CI gate blocks future regressions from merging. | Low        | ~~Must-have~~ **Done**        |
...
| 6   | ~~**Google OAuth state parameter is JSON in query**~~        | **Resolved May 16, 2026.** `google-auth.guard.ts` now generates a `randomBytes(16)` nonce on initiation, stores it in a 5-min httpOnly SameSite=Lax cookie (`oauth_nonce`), and embeds it in the `state` JSON. On callback, nonce is read from cookie, compared to `state.nonce`, cookie cleared; mismatch throws `UnauthorizedException` before Passport runs. Commit `cc29b0c`. | ~~Should-have~~ **Done** |
...
| 8   | **Google OAuth URL versioning mismatch — 404 in production** | **Resolved May 18, 2026.** Frontend was calling `/api/auth/google` but URI versioning requires `/api/v1/auth/google`. Fixed in `CustomerLoginModal.tsx` and `LoginDialog.tsx` to construct correct `/v1/` URL. `google.strategy.ts` fallback URL also corrected.                                                                                                                  | ~~Must-have~~ **Done**   |
| 9   | **OTP codes and phone numbers visible in Cloud Run logs**    | **Resolved May 18, 2026.** `auth.service.ts` OTP `console.log` (~line 250) and Twilio phone log (~line 127) now gated behind `if (process.env.NODE_ENV !== 'production')`. Production logs no longer expose verification codes or customer phone numbers.                                                                                                                         | ~~Must-have~~ **Done**   |
...
| 6   | ~~**localStorage cart serialization on every state change**~~ — **RESOLVED May 18, 2026.** `CartContext` now debounces writes to 500ms (coalesces rapid add/remove bursts). Immediate flush on `unmount` via `pendingCartRef` to prevent data loss on tab close. `cartSaveTimerRef` + `pendingCartRef` pattern.                                                                                                                                                                                                                                                                         | ~~JSON.stringify on every cart mutation — fine for 5-10 items, wasteful at scale.~~                                                               | ~~Debounce localStorage writes to 500ms.~~                                                                                                      | ~~Low~~ **Done**    |
