...
- \@AGENTS.md — Project instruction file for Codex. Describes repo layout, common commands, architecture, conventions, and history.
...
- API base URL: **`/api/v1`** and same-origin in every environment. In development, Vite proxies `/api` and `/socket.io` to the backend origin derived from `VITE_API_URL`. In production, `vercel.json` rewrites `/api/v1/*` to Cloud Run; only Socket.IO connects directly to the backend because Vercel does not proxy WebSockets.
- `VITE_API_URL` in `apps/frontend/.env` supplies the dev proxy target and the direct production Socket.IO origin. `api.ts` never reads it; Axios always uses relative `/api/v1`.
- **Production:** `COOKIE_SAMESITE` is env-driven and defaults to `'none'`; set `COOKIE_SAMESITE=lax` only when the deployed cookie topology has been verified as same-site. This setting does not select the HTTP API route.
...
- **Auth cookie transport** — HTTP API requests are same-origin through relative `/api/v1`: Vite proxies them in development and Vercel rewrites them to Cloud Run in production. `api.ts` does not switch base URLs. Production Socket.IO separately uses the direct `VITE_API_URL` origin with credentials. CSRF double-submit cookies protect state-changing requests.
...
- **Subscription Cache** — The canonical TanStack Query key is `['subscription-status', userId, activeRestaurantId]` in `useFeature.ts`. Both identity dimensions are required so cached entitlements cannot cross users or restaurants during account/tenant switches. Locked navigation for unpaid tiers uses UpgradeModal.
...
- **Translation** (`translation/`) — Job-queue pipeline with sidecar `TranslationJob` table for progress tracking, quota management, and DeepL native glossary support. Platform owns the key via `DEEPL_API_KEY`; there is no `restaurant.deeplApiKey` column. `TranslationService.translateTexts/translateText/translateObject` take **no** `apiKey` parameter. Translation writes happen through (1) fire-and-forget queue pre-warm on menu item/category/option create+update and (2) owner-triggered "Translate All Now" via `POST /api/restaurants/:id/translate-all`. Public `GET /api/menu/public/:id?lang=<code>` only applies already-stored translations through `MenuTranslationReadService`; it never calls DeepL or writes to the DB, and untranslated fields remain canonical until background work fills them. `lang` is validated against `restaurant.targetLanguages`. Free-tier detection: a platform key ending in `:fx` routes to `api-free.deepl.com`. 12 supported locales: EN/BG/RO/DE/ES/FR/IT/ZH/EL/JA/RU/AR.
...
- **HTTP routing:** `api.ts` always uses same-origin `/api/v1`. Vite proxies that path in development and Vercel rewrites it in production. `VITE_API_URL` is reserved for the dev proxy target and direct production Socket.IO connection; do not reintroduce a cross-origin Axios base URL.
