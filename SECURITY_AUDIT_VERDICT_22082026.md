...
**Shape.** Enforcement is conditional on the table *having* a `publicToken` — the same rule service points already follow. A table with a token requires it; a table without one keeps legacy name-based behaviour.
...
| P1-11 | Gate the Neon database to known egress IPs; mark the `production` branch protected.                                                                                                                       | Neon settings                                                              | S      | `allowed_ips` non-empty, branch protected                        |
...
| P2-6  | Namespace R2 keys per restaurant; scope reference counting to the owning tenant; add per-tenant purge                                                                                            | M      |
...
| P2-10 | `expiresAt`/`revokedAt` on `PrintAgentToken` with an eviction path; expiry on staff device enrolment; differentiate JWT TTL by credential class                                                  | M      |
| P2-11 | Surface `StaffPinLoginAudit` in the dashboard and alert on failed-PIN spikes — the data is already collected and currently discarded                                                             | S      |
