...
_This document serves as a reference for recently resolved bugs and UI/UX stabilization efforts across the platform._
...
- **Scroll Jump Bug (Public Menu)**: Fixed a disruptive issue where clicking "Add to Cart" caused the public menu to lose its scroll position and jump to different categories.
  - _Resolution_: Memoized CartContext functions with `useCallback` and refactored the `PublicMenuPage`'s `useEffect` dependency array to only re-fetch menu data when the `restaurantId` or URL query params change, preventing full DOM rebuilds on cart state changes.
...
## Dev Public Menu "Failed to Load" (Jul 17, 2026)

### Summary

Daffi's public menu failed to load on a dev PC's normal browser tab (`"Неуспешно зареждане на менюто..."`) but worked fine in incognito and on mobile.

**Correction — read this first:** the initial diagnosis below (zombie service worker) was wrong for the actual reported case. On the affected machine, DevTools showed **no service worker registered at all** on the `:3001` origin — unregistering/clearing site data did not fix it. The port hardening below is still a real, worthwhile fix for a genuine (separate) SW footgun, but it did not resolve this ticket. See **"Actual Root Cause"** at the bottom of this entry for what actually fixed it.

### Root Cause

`apps/frontend/package.json` served the **production build** (with its Workbox service worker) on **port 3001** — the exact origin `vite dev` also uses:
...
Anyone who ran `npm start`/`npm run preview` once registered the prod SW on `:3001`. That SW persists in any normal browser profile that visited it and:

- serves a cached prod `index.html`/bundle for navigations (`sw.ts` navigation route), and
- serves cached `GET /api/v1/menu/public/*` responses via `NetworkFirst` cache `pos-public-menu-v1` (`sw.ts:32-49`).

It self-perpetuates: the cached prod bundle has `import.meta.env.PROD === true` baked in, so `configureServiceWorker` (`lib/serviceWorkerRegistration.ts`) always takes the production branch and **re-registers itself** — the dev-mode cleanup branch (unregister + cache purge) never runs. Commit `aceb3ffb` (`devOptions.enabled:false`, prod-gated registration, dev cleanup) correctly stops _new_ dev SWs from appearing, but cannot evict a zombie that keeps serving its own old code. Incognito/mobile never installed the SW, so they were unaffected — that split was the diagnostic signal.

CORS/cookie config were investigated and ruled out: the dev flow is same-origin (`/api/v1` + Vite proxy), so no cross-origin request is ever made — the LAN-IP-excluding CORS allowlist and `sameSite:'lax'` cookies never come into play.

### Fix (SW port hardening — real, but not the actual bug)

- **`apps/frontend/package.json`** — moved `start`/`preview` off `:3001` to `:3002` (already in the backend's dev CORS allowlist), so a built PWA's service worker can never again register on the `vite dev` origin.
- **`CLAUDE.md`** — updated the `npm start` port comment.
- One-time manual step tried on the affected dev PC (turned out unnecessary for this bug, but harmless): DevTools → Application → Service Workers → Unregister, then Application → Storage → Clear site data (or run `navigator.serviceWorker.getRegistrations().then(rs=>rs.forEach(r=>r.unregister())); caches.keys().then(ks=>ks.forEach(k=>caches.delete(k)))` in the console), then hard-reload.

### Files Changed (SW hardening)

| File                         | Change                                 |
| ---------------------------- | -------------------------------------- |
| `apps/frontend/package.json` | `start`/`preview` port `3001` → `3002` |
| `CLAUDE.md`                  | `npm start` port comment updated       |

---

### Actual Root Cause — Missing `Cache-Control` on Public Menu API (verified)

The SW cleanup (unregister + "Clear site data") had **no effect** — DevTools confirmed no SW was ever registered on that origin. The next diagnostic step (systematic-debugging: gather evidence before guessing again) was to reproduce with DevTools' Network-tab **"Disable cache"** checkbox ticked — the menu loaded instantly. That isolates the bug to the browser's regular **HTTP disk cache**, a completely different mechanism from the Service Worker Cache API, and one "Clear site data" does not reliably purge.

`curl -D -` against the live backend confirmed it: `GET /api/v1/menu/public/:id/meta` (and the sibling public routes) returned **only a weak `ETag`, no `Cache-Control`, no `Expires`, no `Pragma`**:

```
ETag: W/"2d6d-pZKEMpOn+rzUec+A9PWDtnjGENg"
```

The codebase already has `common/authenticated-no-store.middleware.ts` for exactly this class of bug, but it only fires `if (hasAuthCookie || hasBearerToken)` — a customer scanning a QR code sends **neither**, so it's a complete no-op on every public-menu request. With no explicit cache directive, Chrome is free to serve a previously-cached response from disk on a plain reload without even revalidating. **This is not dev-only** — any real customer's phone browser can silently serve a stale cached menu after the owner edits it, with the same "no error, no visible network failure" signature.

**Fix**: added `@Header('Cache-Control', 'no-store')` (from `@nestjs/common`) to all 5 route handlers in `PublicMenuController` — `getPublicMenu`, `getPublicMenuMeta`, `getCategoryItems`, `getPublicMenuItems`, `getTrendingItems`. Verified live via `curl` against the running dev server (Nest watch-mode hot-reloaded the change) — response now includes `Cache-Control: no-store` alongside the existing ETag.

Regression test added: `apps/backend/src/menu/public-menu.controller.spec.ts` — reads `HEADERS_METADATA` via `Reflect.getMetadata` (same pattern as `super-admin.guard-coverage.spec.ts`) to assert `Cache-Control: no-store` is set on all 5 handlers, so a future edit can't silently drop it.

**Guideline**: any new unauthenticated public GET endpoint (menu, help-content public, etc.) needs an explicit `Cache-Control` header — don't rely on `authenticatedNoStore`, it only covers authenticated traffic.

### Files Changed (actual fix)

| File                                                   | Change                                                   |
| ------------------------------------------------------ | -------------------------------------------------------- |
| `apps/backend/src/menu/public-menu.controller.ts`      | `@Header('Cache-Control', 'no-store')` on 5 routes       |
| `apps/backend/src/menu/public-menu.controller.spec.ts` | **New** — regression test for the header on all 5 routes |
