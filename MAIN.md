...
| Feature | Details |
|---------|---------|
| **Analytics XLSX Export** | Replaced single-sheet CSV export with multi-sheet XLSX workbook. New `lib/analyticsExport.ts` (217 lines). Exports 5 sheets: Summary, Revenue Trend, Top Items, Peak Hours, Category Breakdown. BGN dual-currency columns on all monetary sheets. Excel auto-sizing columns + header styling. Eliminated CSV field name bugs (name/value → category/revenue). |
...
| **Analytics Deep-Dive Full i18n** | All analytics chart labels, tooltips, axis titles, period selectors, export button text, upgrade prompts wired to `t()` across EN/BG/RO. ~40 new i18n keys. Custom date range validation messages localized. Monetary values use `formatEuro()`/`formatBgn()`. |
