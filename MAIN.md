...
| **Currency** | EUR only across menu prices, imports, payments, and exports |
...
├── HelpContentModule (help/FAQ CMS — public GET by section/locale, super-admin CRUD + reorder)
...
| Feature                     | Details                                                                                                                                                                                                                                                                                                             |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Shared Currency Utility** | `lib/currency.ts` provides `formatEuro()` for EUR-only prices throughout checkout, cart, and payment flows. |
...
| **Horizontal Item Cards**   | `ItemWithOptions` redesigned to horizontal layout (image left, content right). Prices displayed in EUR. Pill-shaped "+ Add" buttons (`rounded-full`) replace full-width solid blue "ADD TO CART" buttons. Compact form factor suitable for mobile 375px viewport. |
...
### Post-Roadmap Additions (Shipped May 19-22, 2026)

| Feature                   | Details                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Analytics XLSX Export** | Replaced single-sheet CSV export with multi-sheet XLSX workbook. New `lib/analyticsExport.ts` (217 lines). Exports 5 sheets: Summary, Revenue Trend, Top Items, Peak Hours, Category Breakdown. EUR columns on all monetary sheets. Excel auto-sizing columns + header styling. Eliminated CSV field name bugs (name/value → category/revenue).                                                                                                                                                                                                                                                                                                                                                                                           |
| **Pricing Page Redesign** | Full rewrite of `PricingPage.tsx` (~461 lines). Annual billing toggle (15% discount). Correct tier prices: FREE €0 / STARTER €15 / PROFESSIONAL €25 / ENTERPRISE €45 monthly. 22-row feature comparison table synced to backend `FeatureFlag` enum. 6-entry FAQ accordion. ~60 i18n keys EN/BG/RO. "Most Popular" badge `whitespace-nowrap` fix for BG/RO. `BadRequestException` instead of plain `Error` for checkout failures — front-end can now show real backend error messages.                                                                                                                                                                                                                                                                   |
| **Subscription UX Fixes** | Active-subscription guard prevents duplicate checkout (`ALREADY_SUBSCRIBED` error → auto-redirect to Stripe Portal). Pricing page shows "Current Plan" badge for owned tier, "Manage in Billing Portal" for lower tiers. Annual billing: 6 Stripe price IDs (monthly + yearly per tier), `billingPeriod` param in checkout. `getSubscriptionDetails()` retrieves period dates, interval, cancel status from Stripe.                                                                                                                                                                                                                                                                                                                                     |
| **Help Center CMS**       | Database-driven help system replacing hardcoded i18n JSON. `HelpContent` Prisma model (section, categoryKey, itemKey, sortOrder, locale EN/BG/RO, title, body, active). NestJS module with 6 endpoints: public `GET /help-content/:section` (grouped by category, ordered), super-admin full CRUD + reorder. Frontend: `HelpCenterPage.tsx` (507 lines) with sub-tabs (Landing FAQ / Dashboard Help), locale tabs (EN/BG/RO), inline create/edit/delete. `LandingFAQ.tsx` on home page fetches from API. `HelpView.tsx` (310 lines) in dashboard fetches from API. Seed: 50+ help items across landing (8 items) and dashboard (42+ items) in all 3 locales, migrated from current i18n values. Seed scripts are idempotent — never wipe existing data. |
| **Seed Safety Guards**    | Three-layer protection on `seed.ts`: (1) production check — refuses if `NODE_ENV === 'production'`, (2) remote DB check — warns on non-localhost host, (3) user count > 5 check — refuses unless `FORCE_SEED_WIPE=true`. `seed-help-content.ts` checks `existing > 0` before inserting. `seed-help-only.ts` is single-purpose, zero destructive ops. `seed-demo-restaurants.ts` uses upsert pattern throughout.                                                                                                                                                                                                                                                                                                                                         |
| **Prisma PgBouncer Fix**  | Configured Prisma connection pool for Neon PgBouncer transaction mode compatibility. `PrismaService` constructor added with `super({ log: ['warn', 'error'] })` for pool exhaustion visibility in Cloud Run logs.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **QR Print Layout Fixes** | Improved printable QR code templates: consistent margins, proper padding, i18n-wired template labels and button text for Classic/Premium/Minimal layouts.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |

### Post-Roadmap Additions (Shipped May 23-24, 2026)

| Feature                                     | Details                                                                                                                                                                                                                                                                                                               |
| ------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **XLSX Import/Export Roundtrip**            | Menu export to `.xlsx` (3 sheets: Categories, Items, Options) with styled headers + auto-sized columns. Import from XLSX via file picker, parsed through same import DTO pipeline as JSON OCR import. Enables bulk menu editing in Excel/Google Sheets.                                                               |
| **Public Menu Footer**                      | Configurable footer on public menu with restaurant address, social media icons (Facebook, Instagram, TikTok, Website), contact info, and "Powered by QR Menu" credit. Owner configures social links in Settings → Branding. `footerEnabled` toggle on Restaurant.                                                     |
| **FREE Tier Restrictions**                  | Revenue and AOV summary cards hidden from FREE tier (replaced with upgrade CTA cards). Analytics deep-dive already gated `analytics:full`. Payments/staff features remain gated at service layer. Basic cards (orders, tables, feedback) remain visible.                                                              |
| **Subscription/SaaS Polish**                | Duplicate subscription prevention (`ALREADY_SUBSCRIBED` → portal redirect). Unified TanStack Query cache key `['subscription-status']` across all components. Annual billing with 6 Stripe price IDs. Webhook timestamp-gate race protection.                                                                         |
| **Deploy & Schema Fixes**                   | `prisma db push` removed from container startup — schema managed locally only. Stripe webhook raw body fix (`req.rawBody` → `req.body`). `stripe-webhook.bat` for local Stripe CLI webhook forwarding.                                                                                                                |
| **Onboarding Wizard Overhaul**              | 6-step restaurant creation wizard: Owner Name → Plan Selection (tier-aware Stripe Checkout for paid tiers) → Restaurant Details (name, country, timezone, logo) → Stripe Connect (conditionally skipped for FREE) → Table Setup (bulk create with prefix + count) → QR Preview & Done.                                |
| **Table Status Simplification**             | Reduced from 4 states (empty/waiting/occupied/paid) to 3 (empty/occupied/paid). "Waiting" removed — required subjective waiter judgment. Auto-close PAID sessions after 5 minutes via cron. Color coding: Empty=gray, Occupied=amber, Paid=green.                                                                     |
| **Dashboard Purple/Violet Luxury Redesign** | Complete dashboard UI redesign with dark purple/violet palette, Tailwind v4 migration, vertical dark sidebar with violet accent indicators, glassmorphism summary cards with violet glow borders, Recharts restyled with purple theme palette, dark mobile bottom nav.                                                |
| **Staff Attribution & Itemized Bills**      | `Order.staffUserId` + `Order.source` (POS/QR) track who created each order. `OptionalJwtAuthGuard` captures staff identity on public endpoints when JWT present, rethrows JWT errors (expired/malformed) instead of silently falling through. Itemized bill shows staff name per order. `OrderSource` enum in Prisma. |
| **Table Zones/Sections**                    | `RestaurantTable.zone` field for grouping tables into sections (Main Floor, Terrace, Bar, VIP Room). Zone filter dropdown on table grid and POS table select. Optional zone grouping in LiveTablesView. Zone column in TableView with inline editing.                                                                 |
| **Analytics Deep-Dive Full i18n**           | All analytics chart labels, tooltips, axis titles, period selectors, export button text, upgrade prompts wired to `t()` across EN/BG/RO. ~40 new i18n keys. Custom date range validation messages localized. Monetary values use `formatEuro()`.                                                        |
...
- Name, description, price (EUR), allergens (JSON array), dietary tags
...
[{ "name": "Medium Well", "priceModifier": 0.0 }]
```
...
| `table:status-changed` | Server → Staff    | Table session status changed (empty/occupied/paid)     |
...
9. **EUR only** — all product prices and reports use EUR. Non-EUR imports are rejected, with no runtime currency conversions.
